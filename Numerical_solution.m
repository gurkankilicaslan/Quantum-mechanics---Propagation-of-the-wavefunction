...
for t = 1 : 1000000
    ...
    if (mod(t,100) == 1)
        if (fig.CurrentCharacter == 'h') 
            ...
            display(['Measured energy is ' num2str(value/(pi^2*hbar^2/2/m/L^2))]);
        ...
        plot(L*(0:N+1)/(N+1),[0; conj(f).*f; 0],"r",'LineWidth',3);
        axis([0 L 0 0.04]);
        set(gca,'FontSize',20);
        xlabel('Position');
        ylabel('|\Psi|^2');
        title("Numerical solution-propagation of the wavefunction")
        grid on
        drawnow;
    end
end
